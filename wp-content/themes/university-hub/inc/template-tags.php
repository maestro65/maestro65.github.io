	...
	function university_hub_entry_footer() {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			if ( true === $show_meta_comment ) {
				...
				comments_popup_link( esc_html__( 'Комментировать', 'university-hub' ), esc_html__( '1 Комментарий', 'university-hub' ), esc_html__( '% Комментариев', 'university-hub' ) );
				echo '</span>';
			}
		}
		...
		edit_post_link( esc_html__( 'Изменить', 'university-hub' ), '<span class="edit-link">', '</span>' );
	}
endif;
