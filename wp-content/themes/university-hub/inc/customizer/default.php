...
if ( ! function_exists( 'university_hub_get_default_theme_options' ) ) :
	...
	function university_hub_get_default_theme_options() {
		...
		$defaults['ticker_title']          = esc_html__( 'Новости:', 'university-hub' );
		...
		// Footer.
		$defaults['copyright_text']        = esc_html__( 'Copyright &copy; Все права защищены.', 'university-hub' );
		$defaults['show_social_in_footer'] = false;

		// Blog.
		$defaults['excerpt_length']     = 40;
		$defaults['read_more_text']     = esc_html__( 'Подробнее', 'university-hub' );
		...
		$defaults['news_and_events_ntitle']    = esc_html__( 'Новости', 'university-hub' );
		$defaults['news_and_events_nnumber']   = 2;
		$defaults['news_and_events_ncategory'] = 0;
		$defaults['news_and_events_etitle']    = esc_html__( 'События', 'university-hub' );
		$defaults['news_and_events_enumber']   = 4;
		$defaults['news_and_events_ecategory'] = 0;

		// Homepage Call To Action.
		$defaults['cta_title']               = esc_html__( 'Присоединяйтесь к нашему университету', 'university-hub' );
		...
		$defaults['cta_primary_button_text'] = esc_html__( 'Узнать больше', 'university-hub' );
		$defaults['cta_primary_button_url']  = '#';
		$defaults['cta_secondary_button_text'] = esc_html__( 'Онлайн тур', 'university-hub' );
		$defaults['cta_secondary_button_url']  = '#';

		// Homepage Latest News.
		$defaults['latest_news_title']          = esc_html__( 'Последние новости', 'university-hub' );
		...
		$defaults['featured_slider_read_more_text']      = esc_html__( 'Подробнее', 'university-hub' );
